...
function Registeration(){
  ...
  function RegisterCustomerApi(event) {
    ...
    const data = { username: username,first_name:first_name,last_name:last_name,dob:dob,email:email,address:address, password: password,is_customer:true };
    Axios.post(url, data)
      .then((res) => {
  ...
  <div className="container cc" id="container">
    ...
    <form action="/login" method="POST">
        ...
        <Link to="/">
        <button type="submit" className="btn bb" onClick={RegisterCustomerApi}>REGISTER</button>
        </Link>
      </form>
    </div>
